document.addEventListener('DOMContentLoaded', () => {
    ...
    let cart = [];
    let cartTotal = 0;
    let sales = [];  // Armazena as vendas para o relatório
    ...
    function addToCart(name, price) {
        ...
        recordSale(name, price);  // Registra a venda para o relatório
        updateCart();
    }
    ...
    // Função para registrar a venda no histórico de vendas
    function recordSale(name, price) {
        sales.push({ name, price });
    }
    ...
    checkoutBtn.addEventListener('click', () => {
        ...
        const url = `https://api.whatsapp.com/send?phone=558388660293&text=Pedido:%0A${message}%0A${totalMessage}%0AEndereço: ${encodeURIComponent(address)}`;
    ...
    // Função para enviar o relatório diário automaticamente às 23:00
    function sendDailyReport() {
        ...
        if (currentHour === 23 && currentMinutes === 0) {
            if (sales.length > 0) {
                const salesReport = sales.map(item => `${item.name} - R$ ${item.price}`).join('%0A');
                const totalSales = sales.reduce((sum, item) => sum + parseFloat(item.price), 0).toFixed(2);
                const message = `Relatório de Vendas:%0A${salesReport}%0ATotal Geral: R$ ${totalSales}`;
                const url = `https://api.whatsapp.com/send?phone=558388660293&text=${message}`;

                // Abre o WhatsApp para enviar o relatório
                window.open(url, '_blank');

                // Limpa o histórico de vendas após o envio do relatório
                sales = [];
            } else {
                console.log('Nenhuma venda registrada para o relatório.');
            }
        }
    }
    ...
    // Verifica o horário a cada minuto e envia o relatório diário às 23:00
    setInterval(sendDailyReport, 60000); // 60.000 ms = 1 minuto
});
